...
from torch_geometric.loader import DataLoader
# 注意：在非常旧的 PyG 版本中可能是 from torch_geometric.data import DataLoader
from net.braingnn import Network
from imports.utils import train_val_test_split
from sklearn.metrics import classification_report, confusion_matrix
...
################## Define Dataloader ##################################
# 将 path 修改为当前项目下的 data 目录
# ... 之前的 import 语句保持不变 ...

# 找到原本执行逻辑开始的地方，比如定义 path 的地方
if __name__ == '__main__':
    # 1. 路径修正：指向 data/ABIDE
    # path = os.path.join(os.getcwd(), 'data', 'ABIDE')
    path = opt.dataroot
    name = 'ABIDE'
    ...
    # test_dataset = dataset[te_index]以下6行为csdn修改
